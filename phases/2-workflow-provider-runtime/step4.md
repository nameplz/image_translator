...
1. checkpoint store abstraction을 구현하라.
   ...
   - checkpoint 저장 state는 `JSON-serializable` 값과 `model_dump(mode="json")` 가능한 Pydantic DTO dump로 제한하라.
...
2. resume orchestration을 구현하라.
   ...
   - `CancelledError`는 `WorkflowCancelled` 예외 또는 serialized `JobStatus.cancelled` 상태로 변환하라.
...
- 취소를 실패 상태로 집계하지 마라. Reason: 사용자 취소는 serialized `JobStatus.cancelled` 상태로 별도 처리해야 한다.
